...
一个可以在 bukkit 服务器内 @人的插件。在聊天栏中 `@ + 用户名` 或直接 `输入用户名` 即可在公屏中提及某人。且附带勿扰模式和显示昵称等功能。
...
###### Tips:您可通过禁用某玩家或Group的`at.mention`权限实现 启用/禁用 此玩家/群组@人的权限
## 指令
...
当 OP 输入  `/at isprefix off` 时，玩家仅需输入用户名即可提及某人(如图左下角)。
